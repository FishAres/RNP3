...
function get_model(args)
    ...
    return Encoder |> gpu, Hx |> gpu, Ha |> gpu, Hx_bounds, Ha_bounds
end
...
function run_training_thingy(scale_offset, λ, λpatch, η)
    ...
    Encoder, Hx, Ha, Hx_bounds, Ha_bounds = get_model(args)
    for epoch in 1:20
        ls = train_model(opt, ps, train_loader; epoch=epoch, logger=nothing)
    end
...
ho = @hyperopt for i = 10,
    ...
    cost = run_training_thingy(scale_offset, λ, λpatch, η)
