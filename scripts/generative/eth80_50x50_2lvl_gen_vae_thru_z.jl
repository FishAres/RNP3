...
function get_fstate_models(θs, Hx_bounds; args=args, fz=args[:f_z])
    ...
    f_state = ps_to_RN(get_rn_θs(Θ[2], args[:π], 2 * args[:π]); f_out=elu)
...
l_fx = get_rnn_θ_sizes(args[:π], 2args[:π]) # μ, logvar
...
alias = "2lvl_double_H_eth80_50x50_vae_v01_thru_noise"
