...
function get_scrambled_mnist(digit_batches; args=args)
    ...
    return fast_concat(imgs)
end

function fast_concat(xs)
    out = zeros(Float32, size(xs[1])..., length(xs))
...
train_data, test_data = get_scrambled_mnist.((train_batches, test_batches))
...
alias = "double_H_mnist_2to4_rand_locs_generative_v0"
