...
function get_models(θsz, θsa; args=args)
    (Enc_za_z, f_state, err_rnn, Dec_z_x̂, Enc_ϵ_z,), z0 = get_fstate_models(θsz; args=args)
    (Enc_za_a, f_policy, Dec_z_a,), a0 = get_fpolicy_models(θsa; args=args)
...
function train_model(opt, ps, train_data; args=args, epoch=1, logger=nothing, D=Uniform(-1.0f0, 1.0f0))
...
function test_model(test_data; D=Uniform(-1.0f0, 1.0f0))
