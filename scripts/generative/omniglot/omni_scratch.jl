...
modelpath = "saved_models/gen_2lvl/omni_2lvl_convH_v01_reslayers_H/add_offset=true_asz=6_bsz=64_esz=32_glimpse_len=4_img_channels=1_imzprod=784_norm_groups=8_scale_offset=2.0_scale_offset_sense=3.2_seqlen=4_α=1.0_β=0.05_η=0.0001_λ=1e-6_λf=0.167_λpatch=0.0001_π=64_200eps.bson"
...
function z_loss(μ, logvar, x, r)
    ...
    klqp = kl_loss(μ, logvar)
    rec_loss = Flux.mse(flatten(out), flatten(x); agg=sum)
    loss = args[:α] * (rec_loss + args[:λpatch] * Lpatch) + args[:β] * klqp
    return loss, out
    # return rec_loss + args[:λpatch] * Lpatch, out
end

function z_descent(opt, μ, logvar, x; n_iter=300, anneal=false, λ=0.1, anneal_every=10)
    rs = [rand(args[:D], args[:π], args[:bsz]) for _ in 1:n_iter] |> gpu
    mean, lvar = copy(μ), copy(logvar)
    Ls, preds, etas = [], [], []
    ps = Flux.params(mean, lvar)
    for i in 1:n_iter
        loss, grad = withgradient(ps) do
            loss_, out = z_loss(mean, lvar, x, rs[i])
            Zygote.ignore() do
                push!(preds, cpu(out))
            end
            loss_
        end
        anneal && (i % anneal_every == 0) && (opt.eta = λ * opt.eta)
        Flux.update!(opt, ps, grad)
        push!(Ls, loss)
        push!(etas, opt.eta)
        # _, out = z_loss(mean, lvar, x, rs[i])
        # push!(preds, cpu(out))
    end
    return Ls, preds, etas
end

x = sample_loader(val_loader)
μ, logvar = Encoder(x)

opt = ADAM(0.1)
ls, preds, etas = z_descent(opt, μ, logvar, x; n_iter=200, anneal=true, λ=0.9, anneal_every=2)

r = rand(args[:D], args[:π], args[:bsz]) |> gpu
_, out1 = z_loss(μ, logvar, x, r)

ind = 0
begin
    ind = mod(ind + 1, args[:bsz]) + 1
    ploss = plot(ls, label="loss")
    plot!(etas * 10000, label="η * 1e4")
    p1 = plot_digit(cpu(out1)[:, :, 1, ind], title="orig. reconstruction")
    p2 = plot_digit(cpu(preds[end])[:, :, 1, ind], title="corrected")
