...
function get_fstate_models(Θ, Hx_bounds; args=args, fz=args[:f_z])
...
"one hypernet, no heads"
function get_fpolicy_models(Θ, Ha_bounds; args=args)

    Enc_za_a = Chain(HyDense(args[:π] + args[:asz], args[:π], Θ[9], sin), flatten)
    f_policy = ps_to_RN(get_rn_θs(Θ[10], args[:π], args[:π]); f_out=sin)
    Dec_z_a = Chain(HyDense(args[:π], args[:asz], Θ[11], sin), flatten)
...
function get_models(θs; args=args, Hx_bounds=Hx_bounds, Ha_bounds=Ha_bounds)
    (Enc_za_z, f_state, Dec_z_x̂), z0 = get_fstate_models(θs, Hx_bounds; args=args)
    (Enc_za_a, f_policy, Dec_z_a,), a0 = get_fpolicy_models(θs, Ha_bounds; args=args)
...
function full_sequence(z::AbstractArray, x; args=args, scale_offset=args[:scale_offset])
    θs = H(z)
    models, z0, a0 = get_models(θs; args=args)
...
H = Chain(
    ...
    LayerNorm(64, elu),
    Split(
        [Dense(64, θ) for θ in [Hx_bounds; args[:π]; Ha_bounds; args[:asz]]]...,
    )) |> gpu
...
alias = "omni_2lvl_single_H_split_heads_v0"
